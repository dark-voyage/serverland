...
composer.action(/accept_form_(.*)/gi, async (ctx) => {
	...
	await ctx.telegram.sendMessage(applicant, message.form_accept(ctx), {
		parse_mode: "HTML",
...
composer.action(/decline_form_(.*)/gi, async (ctx) => {
	const applicant = ctx.match[1];
	await ctx.telegram.sendMessage(applicant, message.form_decline(ctx), {
		parse_mode: "HTML",
